...
test('public 1.0.0 metadata follows the build naming scheme', () => {
  assert.deepEqual(validateVersionInfo(versionInfo), versionInfo);
  assert.equal(versionInfo.build, '1.0.0+4.103R');
  assert.equal(versionInfo.artifact, 'panel.dmg');
});

test('runtime metadata adds only a validated positive patch number', () => {
  assert.equal(runtimeVersionInfo(versionInfo, 2).build, '1.0.0+4.103Rp2');
  assert.equal(runtimeVersionInfo(versionInfo, 0).build, '1.0.0+4.103R');
  assert.equal(runtimeVersionInfo(versionInfo, '2').build, '1.0.0+4.103R');
});
...
test('metadata rejects mismatched channels, artifacts, and compatibility', () => {
  ...
  assert.throws(
    () => validateVersionInfo({ ...versionInfo, livePatchCompatibility: '>=1.0.1' }),
    /compatibility/,
  );
});
...
test('dashboard exposes public version and triple-click build details', () => {
  ...
  assert.match(html, /id="app-version"[^>]*>1\.0\.0</);
