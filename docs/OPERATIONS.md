...
| Current build output | `dist/1.1.0/1.1.0+1.7D` |
...
`1.1.0-alpha.1` while keeping the displayed App version `1.1.0`. This ensures
Electron considers the later stable `1.1.0` newer than the developer test.
...
```bash
hdiutil verify 'dist/1.1.0/1.1.0+1.7D/panel.dmg'
unzip -tq 'dist/1.1.0/1.1.0+1.7D/panel.zip'
codesign --verify --deep --strict --verbose=2 'dist/1.1.0/1.1.0+1.7D/mac-arm64/Panel.app'
shasum -a 256 'dist/1.1.0/1.1.0+1.7D/panel.dmg'
shasum -a 256 'dist/1.1.0/1.1.0+1.7D/panel.zip'
```
...
ditto 'dist/1.1.0/1.1.0+1.7D/mac-arm64/Panel.app' /Applications/Panel.app
